...
module.exports = {
    ...
    fetchGithubCommitCountSinglePage: function(repo, page = 1, prevCount = 0) {
        return this.fetchGithub(`repos/stevenliuyi/${repo}/commits`, false, page).then(
            (data) =>
                (Array.isArray(data)
                    ? data.filter((d) => d.author != null && d.author.login === 'stevenliuyi').length
                    : 0) + prevCount
        )
    },

    fetchGithubCommitCount: function(repo) {
        return this.fetchGithub(`repos/stevenliuyi/${repo}/commits`, true).then((link) => {
            if (link == null) {
                // only one page
                ...
                for (let p = 2; p <= lastPage; p++) {
                    promise = promise.then((count) => this.fetchGithubCommitCountSinglePage(repo, p, count))
                }
                return promise
            }
        })
    ...
    fetchWikidata: function(query) {
        return fetch(`https://query.wikidata.org/sparql?query=${encodeURIComponent(query)}&format=json`)
            .then((res) => res.json())
            .catch((err) => {
                console.log(err)
                return null
    ...
    fetchAirportLocations: function(codes) {
        const codeString = codes.map((x) => `"${x}"`).join(' ')
        const query = `SELECT DISTINCT ?code ?coord WHERE { VALUES ?code {${codeString}} ?airport wdt:P238 ?code; wdt:P625 ?coord }`
        return this.fetchWikidata(query).then((data) => {
            return data != null ? data.results.bindings : null
        })
    }
}
